...
class BoxesandGridsGame():
    ...
    '''
    def player2(self):
        temp_h=deepcopy(self.boardh)
        temp_v=deepcopy(self.boardv)
        '''
    ...
    def next_possible_testMoves(self,move,horizontal,vertical):
        #make the move true if the last move is not true to be true in the psuedo list
        if(move[2]==1):
            horizontal[move[0]][move[1]]=True
        elif(move[2]==0):
            vertical[move[0]][move[1]]=True
        else: 
            print ("Invalid move")
        next_moves=[]
        ...
        for x in range (7):
            for y in range(6):
                if(horizontal[x][y]==False):
                    next_moves.append([x,y,1]); # append all horizontal moves
                
        for x in range (6):
            for y in range(7):
                if(vertical[x][y]==False):
                    next_moves.append([x,y,0]); # append all horizontal moves
        
                
        return next_moves 

    def alphabeta(self,depth = 4, alpha=-float('inf'), beta=float('inf'), is_max = True, player = True, move = [0,0,0],horizontal=[],vertical=[],p2score=0,p1score=0):
        inf = float('inf')
        children = self.next_possible_testMoves(move,horizontal,vertical)
        if len(children) == 0 or depth == 0 or self.game_ends(horizontal,vertical):
            return [move,p2score-p1score]
        ...
        if is_max:
            ...
            for move in children:
                ...
                temp = self.alphabeta( depth - 1, alpha, beta, False, player, move,tempstate[0],tempstate[1],p2score+tempstate[2],p1score)
